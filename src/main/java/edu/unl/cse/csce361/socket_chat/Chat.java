...
public class Chat {
	...
	private Cipher cipherBehavior;

	public Chat() {
		setLocale(Locale.getDefault());
		cipherBehavior = CipherFactory.createCipher();
		socket = connect(new Scanner(System.in));
	}

	private void setLocale(Locale locale) {
		bundle = ResourceBundle.getBundle("socketchat", locale);
		Set<String> culledKeySet = bundle.keySet().stream().filter(entry -> entry.startsWith("communicate.keyword."))
	 ...
	 */
	private Socket connect(Scanner userInput) {
	...
	private Socket connectAsServer(Scanner userInput) throws IOException {
		...
		// "Host address: ..."
		System.out.println(MessageFormat.format(bundle.getString("connection.info.hostAddress.0.1.2.3"), address[0],
	...
	private Socket connectAsClient(Scanner userInput) throws IOException {
		...
		String prompt = MessageFormat.format(bundle.getString("connection.prompt.getHostPort.0.1.2.3"), address[0],
				address[1], address[2], address[3]);
		...
		do {
			...
			try {
				...
				System.out.println(MessageFormat.format(bundle.getString("connection.error.hostNotReady.0.1.2.3.4.5"),
				...
				if (attemptCount < MAXIMUM_CONNECTION_ATTEMPTS) {
					...
					System.out
							.println(MessageFormat.format(bundle.getString("connection.info.reAttempt"), attemptCount));
					socket = null;
				} else {
		...
		while (!haveGoodAddress) {
			...
			try {
				...
				if (tokens.length == 4) {
					...
					System.out.println(
							MessageFormat.format(bundle.getString("connection.error.malformedAddress"), addressString));
				...
				if (message.startsWith(bundle.getString("exception.numberFormat.startOfValueOutOfRangeMessage"))) {
					...
					if ((127 < value) && (value < 256)) {
						// "Note that Java does not have unsigned integers, so subtract 256 from values
						// greater than..."
						System.out.println(
								MessageFormat.format(bundle.getString("connection.error.userAttemptedUnsignedByte.0.1"),
										value, value - 256));
					}
				}
				haveGoodAddress = false;
			...
			try {
				...
				if (port <= 0) {
					...
					throw new InputMismatchException(
							MessageFormat.format(bundle.getString("connection.error.portNumberTooLow"), port));
				}
				if (port >= 2 * (Short.MAX_VALUE + 1)) {
					// "Expected value less than 65536, got ..."
					throw new InputMismatchException(
							MessageFormat.format(bundle.getString("connection.error.portNumberTooHigh"), port));
				}
				haveGoodNumber = true;
	...
	private void communicate() {
		try {
			communicate(new BufferedReader(new InputStreamReader(System.in)),
					new BufferedReader(new InputStreamReader(socket.getInputStream())), System.out,
	...
	private void communicate(BufferedReader localInput, BufferedReader remoteInput, PrintStream localOutput,
			...
			while (keepTalking) {
				try {
					if (myTurnToTalk) {
						...
						remoteOutput.println(encipher(message, userKey));
					} else {
						String encipheredMessage = remoteInput.readLine();
						if (encipheredMessage != null) {
							message = decipher(encipheredMessage, userKey);
	...
	private boolean handleKeyword(String keyword, boolean localMessage, BufferedReader input, PrintStream output) {
		if (keyword.equals(bundle.getString("communicate.keyword.exit"))) {
			...
			if (localMessage) {
				output.println(bundle.getString("communicate.language.selectLanguage"));
				...
				if (userSelection.equals(bundle.getString("communicate.response.1"))) {
					setLocale(Locale.ENGLISH);
				} else if (userSelection.equals(bundle.getString("communicate.response.2"))) {
					setLocale(Locale.CHINESE);
				}
				output.println(bundle.getString("communicate.language.changeSuccess"));
			} else {
				output.println(bundle.getString("communicate.language.remoteSelectLanguage"));
			}
		} else if (keyword.equals(bundle.getString("communicate.keyword.setCipher"))) {
			if (localMessage) {
				userKey = null;
				output.println(bundle.getString("communicate.cipher.selectCipher"));
				String userSelection = null;
				try {
					userSelection = input.readLine();
				} catch (IOException e) {
				...
				if (userSelection.equals(bundle.getString("communicate.response.2"))) {
					cipherBehavior = CipherFactory.createCipher("Base64", null);
					output.println(bundle.getString("communicate.cipher.changeSuccess"));
				} else if (userSelection.equals(bundle.getString("communicate.response.3"))) {
					output.println(bundle.getString("communicate.cipher.setPrivateKey"));
					boolean setKey = false;
					while (!setKey) {
						try {
							userKey = input.readLine();
						} catch (IOException e) {
						}
						if (userKey.length() == 8) {
							setKey = true;
						} else {
							output.println(bundle.getString("communicate.cipher.wrongKeySize"));
						}
					}
					cipherBehavior = CipherFactory.createCipher("DES", userKey);
					output.println(bundle.getString("communicate.cipher.changeSuccess"));
				} else if (userSelection.equals(bundle.getString("communicate.response.1"))) {
					cipherBehavior = CipherFactory.createCipher();
					output.println(bundle.getString("communicate.cipher.changeSuccess"));
				}
			} else {
				output.println(bundle.getString("communicate.cipher.remoteSelectCipher"));
			}
		}
	...
	private String encipher(String plaintext, String key) {
		String ciphertext = cipherBehavior.encipher(plaintext, key);
		if (ciphertext.equals("Error.WrongCipherSet Error84$"))// user will never try to type this.
			return bundle.getString("communicate.cipher.wrongCipher");
		else
			return ciphertext;
	}

	private String decipher(String ciphertext, String key) {
		String plaintext = cipherBehavior.decipher(ciphertext, key);
		if (plaintext.equals("Error.WrongCipherSet Error84$"))
			return bundle.getString("communicate.cipher.wrongCipher");
		else
			return plaintext;
	}
